# 48 LED Binary Counter Trix Gold Pro Python program example:
...
# 220Ω ohm resistor = 48
# jumper wire = approx. 64 or more +2 for the Rasp pi 4 fan

# Note: use two other jumper wires for
# the Raspberry Pi 4 fan, while in use/
# operation.

# 48b LED Binary Counter Trix Python program example:

# This Raspberry Pi 4 Python program allows
# users to have tons of fun, while learning
# how six 8b 74HC595 shift registers work.
...
def binary_bits_inverse():
    ...
    try:
        for i in range(msb[0],lsb[0],-1):
            bin = f'{i:b}'
            print('\n'+str(len(f'{msb[0]-i:b}')),
                  f'bits = Bin: {(i & 0xffffffffffff):048b} =\n\n'
                  f'Hex: -{msb[0]-i:X}\n'
                  f'Oct: -{msb[0]-i:o}\n'
                  f'Dec: -{msb[0]-i:,}')
            for j in range(48):
